...
var bookBlockList = Vue.extend({
      ...
      +'<ul class="list clear" ><li><div class="vertical-img"><a v-link="{path: \'/works\'}"><img src="../img/90x126.png" class="cover"/></a></div><div class="vertical-title"></div></li><li v-for="obj in bookList"><div class="vertical-img"><div class="state_one">审核</div><div class="state_tow">连载</div><img :src="path.rootPath + obj.bookCoverImage" class="cover"  v-link="{path: \'/catalogue/\' + obj.bookId}"  /></div><div class="vertical-title" title="{{obj.bookName}}">{{obj.bookName}}</div></li></ul></div></div> '
      +'</div>'
      +'</div></div></div>'
      ,data: function() {
        return {
